...
def update_entries(session, entries_to_write):
    ...
    update_statement = (
        update(table)
        .where(table.c.property_one == bindparam('property_one'))
        .values(
...
def start():
    ...
    with database_service.scoped_session() as session:
        ...
        # https://dba.stackexchange.com/questions/192599/postgres-difference-between-tuples-inserted-and-live-tuples
        # https://www.postgresql.org/docs/current/mvcc-intro.html
        print('see all dead tuples')
        print('select n_live_tup, n_dead_tup, relname from pg_stat_all_tables;')
        time.sleep(30)
        print('done')
